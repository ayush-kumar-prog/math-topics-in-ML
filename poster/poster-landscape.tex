...
\setbeamertemplate{headline}{
  ...
  \begin{beamercolorbox}[wd=\paperwidth]{headline}
    ...
    \begin{minipage}[c]{0.10\paperwidth}
      \hspace*{0.6cm}\includegraphics[height=4.0cm]{Manchester_Logo}
    \end{minipage}%
...
\eyebrow{Thesis}
{\color{uomgrey}\textbf{\normalsize For modern training procedures the population-risk curve exhibits \emph{two} descents,} separated by a sharp peak at the \kw{interpolation threshold} at which the procedure first fits the training set exactly.}\\[0.5ex]
...
The classical bias-variance trade-off bundles \emph{approximation + part of estimation} into \kw{bias} and the rest into \kw{variance}; \emph{incomplete}, not wrong. The \kw{paper's pivot:} treat the entire procedure \(\T\) -- architecture, optimiser, training time, regularisation, and label policy -- as the single object whose EMC matters.

\vspace{0.2ex}
\eyebrow{Term-by-term behaviour at the peak}
\footnotesize
\textcolor{accentteal}{\textbf{Optimisation}} declines with training time and width; \textcolor{accentgold}{\textbf{estimation}} \emph{spikes} at \(\mathrm{EMC}\!\approx\! n\); \textcolor{accentblue}{\textbf{approximation}} declines with hypothesis-class size.
\end{block}
...
i.e.\ the largest sample size on which \(\T\) drives train error below \(\varepsilon\). Note that \(\mathrm{EMC}\) depends on the optimiser, training time, augmentation, and labels, not solely on the architecture.
...
\begin{column}{0.325\linewidth}
\begin{block}{\sectnum{5}Sample-wise descent: increasing \(n\) can locally raise test risk}
\centering
...
\footnotesize
ResNet18, CIFAR-10, \kw{15\% label noise}, augmentation, Adam (lr 1e-4), 4k epochs. Pattern identical under SGD, on CIFAR-100, and at \kw{0\% noise} (Fig.~4a). Sample-wise (Fig.~3): a \kw{4.5\(\times\) data increase} (4k\(\to\)18k) raises test loss for \(d_{\text{model}}\!\in\![50,100]\). Paper uses \kw{\(\varepsilon\!=\!0.1\)}.
...
{\scriptsize
[1]~Nakkiran, Kaplun, Bansal, Yang, Barak, Sutskever. \emph{Deep Double Descent.} arXiv:1912.02292 (2019). \;
